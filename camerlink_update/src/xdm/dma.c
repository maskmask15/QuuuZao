...
//	printf("GaussianBlur_s64_FPGA_while src:0x%x\n",(void *)src->data);
//	printf("GaussianBlur_s64_FPGA_while dst:0x%x\n",(void *)dst->data);
	status = XAxiDma_SimpleTransfer(get_Axidma(DeNU2_DMA_DEV_ID), (UINTPTR) src->data,
			lrngth, XAXIDMA_DMA_TO_DEVICE);
	if (status != XST_SUCCESS) {
		printf("====================GaussianBlur_s64_FPGA_while XAXIDMA_DMA_TO_DEVICE fail\n");
		return XST_FAILURE;
	}
	...
	if (status != XST_SUCCESS) {
		printf("====================GaussianBlur_s64_FPGA_while XAXIDMA_DEVICE_TO_DMA fail\n");
		return XST_FAILURE;
	}
